...
const setFormatter = async () => {
  const output = await execa.sync(executablePath, ['-formatter=json-stream'], { timeout: 5*1000, reject: false });
  switch (output.code) {
  case 0:
    formatter = 'json-stream';
    break;
  case 1:
    console.log('JSON Stream is not supported by this Revive version. Default formatter is used');
    formatter = 'default';
    break;
  default:
...
// Experimental (https://github.com/morphy2k/revive/blob/json-stream/formatter/json-stream.go)
